...
residual  = 1;
sortwrd   = 0; % sortwrd   = 1; % Amanda changed this!
...
smallnbasis = 13;%65; SARAH CHANGE
...
%    a column for each climate zone effect. Add a dummy constraint
%    observation
...
zonenames = ['Atlantic   ';'Pacific    ';'Continental'; 'Arctic     '];
prednames = ['Mean       ';zonenames];
for j=1:p
...
yhatmat  = eval_fd(monthtime, yhatfdobj);
ymat     = eval_fd(monthtime, tempfd);
...
monthbasismat = eval_basis(monthtime, monthbasis);
y2cMap = monthbasismat';

[betaestcell, yhatfdobj, betastderrcell, bvar, c2bmap] = ...
    fRegress(tempfd36, xfdcell, betacell, y2cMap);

%  plot regression functions with confidence limits

for j=1:p
    plotbeta(betaestcell{j}, betastderrcell{j}, months)
